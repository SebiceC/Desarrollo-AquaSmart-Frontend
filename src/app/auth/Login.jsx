...
const Login = () => {
  ...
  const handleRequestNewToken = async () => {
    ...
    try {
      ...
      openModal("ERROR", "Error al generar token, intente mas tarde!.", "ACEPTAR", handleConfirm)
      if (err.response) {
  ...
  const handleTokenSubmit = async () => {
    ...
    try {
      ...
      if (response.data.token) {
        ...
        openModal("INICIO DE SESIÓN EXITOSO", "Bienvenido a la plataforma.", "CONTINUAR", () => navigate("/home"))
      } else {
      ...
      <div className="w-full lg:w-[60%] h-full min-h-screen flex flex-col">
            ...
            <div className="w-[83%] sm:w-[70%] lg:w-[60%] bg-[#ebfdfd] p-6 border-1 shadow-lg border-[#003F88] rounded-lg flex flex-col items-center">
              ...
              <form onSubmit={handleLogin} className="flex flex-col items-center w-full">
                ...
                <div className="flex flex-col items-center gap-2 mt-2">
                  <a href="/forgotPassword" className="font-semibold text-sm hover:underline">
                    OLVIDÉ MI CONTRASEÑA
                  </a>
                  <a href="/preRegister" className="font-semibold text-sm hover:underline">
                    SOY USUARIO NUEVO
                  </a>
                </div>
                <button
        ...
        <div className="w-full flex flex-col sm:flex-row px-10 py-4 sm:py-0 gap-3 bg-[#c9ebd7] h-auto sm:h-15 justify-between items-center mt-auto">
          <div className="flex items-center gap-2">
            ...
            <span className="text-md font-medium text-gray-700">soporte@aquasmart.com</span>
          </div>
          <div className="flex items-center gap-2">
            <IoLogoWhatsapp size={24} className=" text-[#2ba19b]" />
            <span className="text-md font-medium text-gray-700">+57 300 123 4567</span>
          </div>
        </div>
      </div>
    </div>
