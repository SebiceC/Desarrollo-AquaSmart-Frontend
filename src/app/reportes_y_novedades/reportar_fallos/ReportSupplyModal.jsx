...
const ReportSupplyModal = ({ showModal, onClose, onSuccess, API_URL }) => {
    ...
    const [selectedPlot, setSelectedPlot] = useState("");  // Estado para el predio seleccionado
    const [selectedLot, setSelectedLot] = useState("");    // Estado para el lote seleccionado
    const [loadingLots, setLoadingLots] = useState(false); // Estado para indicar carga de lotes
    ...
    useEffect(() => {
        if (showModal) {
            ...
            setSelectedPlot("");  // Resetear el predio seleccionado
            setSelectedLot("");   // Resetear el lote seleccionado
            setLots([]);         // Limpiar los lotes
    ...
    const handleSubmit = async () => {
        ...
        try {
            ...
            await axios.post(
                `${API_URL}/communication/water-supply-failure-report`,
                {
                    observations: observations.trim(),
                    plot_id: selectedPlot,
                    lot: selectedLot || null,  // Incluir el lote seleccionado si existe, pero no obligatorio
                },
                {
                    headers: { Authorization: `Token ${token}` },
                }
    ...
    return (
        <div className="fixed inset-0 flex items-center justify-center backdrop-blur-xs z-50">
            <div className="bg-white p-6 rounded-lg shadow-lg text-center w-[90%] sm:w-[500px] z-50">
                ...
                {/* Select para los lotes (siempre visible, pero bloqueado si no se selecciona un predio) */}
                <div className="mb-4">
