...
const HistorialUserPredioDetail = () => {
  ...
  // Validar si el usuario es propietario del predio con manejo mejorado de errores
  useEffect(() => {
    const validateOwnership = async () => {
      try {
        ...
        if (!token) {
          ...
          // Continuamos sin validación en caso de error de token
          return;
        }
        
        // Obtener datos del usuario logueado usando axios
        try {
          ...
          try {
            const plotResponse = await axios.get(`${API_URL}/plot-lot/plots/${id_plot}`, {
              headers: {
                'Authorization': `Token ${token}`,
                'Content-Type': 'application/json'
              }
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        ...
        try {
          const response = await axios.get(`${API_URL}/caudal/flow-measurements/predio/${id_plot}`, {
            headers: {
              'Authorization': `Token ${token}`,
              'Content-Type': 'application/json'
            }
          ...
          // Si hay error de red o servidor, usamos datos de ejemplo temporalmente
          setError('Error al cargar los datos: ' + apiError.message);
        ...
        // No usar datos de ejemplo si no hay datos reales
        if (err.message === 'No existe consumo del predio') {
          setData([]);
        } else {
          // Use mock data only for other types of errors
          setData(generateMockData(groupByOption));
          setShowGraphErrorModal(true);
        }
      } finally {
    ...
    fetchData();
  }, [groupByOption, startDate, endDate, dateValidationError, id_plot, API_URL]);
  ...
  const processDataByTimeRange = (rawData, groupOption) => {
    ...
    try {
      ...
      console.error('Error procesando datos por rango de tiempo:', err);
      return [];
    }
  };
