  ...
  const fetchInfoAsignacion = async () => {
    ...
    try {
      ...
      const response = await axios.get(
        ...
        { 
          headers: { Authorization: `Token ${token}` },
          params: { failure_report: solicitudBasica.id } // Enviar parámetro de filtro
        }
      );
      ...
      if (response.data && response.data.length > 0) {
        ...
        const asignacion = response.data.find(
          item => item.failure_report === solicitudBasica.id
        );
      ...
      if (response.data && Array.isArray(response.data)) {
        ...
        const informesFiltrados = response.data.filter(
          informe => informe.assignment_details && 
                    informe.assignment_details.failure_report === solicitudBasica.id
        );
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                          ...
                          <div className="flex flex-col space-y-1">
                            ...
                            <div className="flex items-center">
                              ...
                              <span className="text-gray-800">{solicitudBasica.failure_report_type || tipoAsignacion || "No especificado"}</span>
                            </div>
                          </div>
                        </div>
...
export default BaseInfoFinalModalR;