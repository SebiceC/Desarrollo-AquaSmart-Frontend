 ...
 * @param {Object} props
 * @param {Array} props.predictionData - Prediction data from API
 * @param {boolean} props.isLoading - Loading state
 * @param {string} props.error - Error message
 * @param {Function} props.onPeriodChange - Callback when prediction period changes
 * @param {Function} props.onGeneratePredictions - Callback to generate predictions
 * @param {string} props.selectedPeriod - Currently selected period ('1', '3', '5')
 * @param {string} props.lotId - ID of the lot for predictions
 * @param {string} props.title - Chart title
 * @param {Object} props.chartRef - Ref for the chart container (for PDF export)
 * @param {boolean} props.isGenerating - Is generating predictions
 * @param {boolean} props.canGenerate - Can generate new predictions
 */
const PredictionLotChartComponent = ({ 
    ...
    try {
      ...
      const result = sortedData.map(item => {
        ...
        return {
          ...
          flowRate: parseFloat(item.consumption_prediction || 0),
  ...
  // Mostrar modal de sin datos cuando no hay datos y no está cargando
  useEffect(() => {
    if (!isLoading && processedData.length === 0 && !error) {
      setShowNoDataModal(true);
    }
  }, [isLoading, processedData.length, error]);
  ...
  const handlePeriodChange = (period, e) => {
    ...
    try {
      ...
      // Notificar el cambio de período al componente padre
  ...
  return (
    <div className="flex flex-col bg-white rounded-lg shadow-md p-6 md:p-10 w-full max-w-4xl mx-auto">
      ...
      >
        <p>
          ¡Ocurrió un error al momento de generar la predicción! {error || 'Error desconocido'}.
          Vuelve a intentarlo más tarde o ponte en contacto con soporte.
        </p>
      </Modal>
      ...
      >
        <p>
          No hay datos de predicción disponibles para el período seleccionado.
          Intenta generar nuevas predicciones.
        </p>
      </Modal>
      ...
      <div className="mb-6">
        ...
        <div className="flex flex-col sm:flex-row justify-between items-center mb-8">
          {/* Selector de período de predicción */}
          <div className="relative mb-4 sm:mb-0">
            <button 
              onClick={togglePeriodSelector}
              type="button"
              className="flex items-center gap-2 bg-green-100 text-green-700 hover:bg-green-200 px-4 py-2 rounded-lg font-medium"
                ...
                {Object.entries(predictionPeriods).map(([key, period]) => (
                  <button
                    ...
                    className={`w-full text-left px-4 py-3 hover:bg-gray-50 first:rounded-t-lg last:rounded-b-lg ${
                      selectedPeriod === key ? 'bg-green-50 text-green-700 font-medium' : 'text-gray-700'
                    }`}
          ...
          {/* Botón de generar predicciones y información del lote */}
            ...
            <div className="flex">
              <div className="ml-3">
                <p className="text-sm text-yellow-700">
                  <strong>Nota:</strong> Las predicciones se basan en modelos de IA entrenados con datos históricos. 
                  Los valores reales pueden variar según condiciones externas y cambios en el uso.
                </p>
              </div>
            </div>
          </div>
        ...
        {isLoading ? (
          <div className="flex items-center justify-center h-full">
            <div className="text-center">
              ...
              <p className="text-gray-500">Generando predicciones con IA...</p>
            </div>
          </div>
        ) : error ? (
          <div className="flex items-center justify-center h-full">
            <div className="text-center">
              <TrendingUp className="mx-auto text-red-400 mb-2" size={48} />
              <p className="text-red-500 mb-2">Error al generar predicciones</p>
          ...
          <div className="flex items-center justify-center h-full">
            <div className="text-center">
              ...
              <p className="text-sm text-gray-400">Selecciona un período para generar predicciones</p>
            </div>
          </div>
        ) : (
          <ResponsiveContainer width="100%" height="100%">
            ...
            >
              ...
              <Line 
                ...
                strokeWidth={3}
                strokeDasharray="8 8"
                dot={{ stroke: '#10B981', strokeWidth: 2, r: 5, fill: 'white' }}
                activeDot={{ stroke: '#059669', strokeWidth: 2, r: 7, fill: '#10B981' }}
              />
            </LineChart>
          </ResponsiveContainer>
        )}
          ...
          <div className="flex justify-center">
            <div className="flex items-center gap-4 bg-gray-50 px-4 py-2 rounded-lg">
              <div className="flex items-center gap-2">
                <div className="w-6 h-0.5 bg-green-500" style={{borderTop: '3px dashed #10B981'}}></div>
                <span className="text-sm text-gray-600">Predicción de consumo mensual</span>
              </div>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
            <div className="bg-blue-50 p-3 rounded-lg text-center">
              ...
              <p className="text-lg font-bold text-blue-800">
                {(processedData.reduce((acc, item) => acc + item.flowRate, 0) / processedData.length).toLocaleString()} L
              </p>
            </div>
            <div className="bg-green-50 p-3 rounded-lg text-center">
              <p className="text-sm text-green-600 font-medium">Consumo Máximo</p>
              <p className="text-lg font-bold text-green-800">
                {Math.max(...processedData.map(item => item.flowRate)).toLocaleString()} L
              </p>
            </div>
            <div className="bg-orange-50 p-3 rounded-lg text-center">
              <p className="text-sm text-orange-600 font-medium">Consumo Mínimo</p>
              <p className="text-lg font-bold text-orange-800">
                {Math.min(...processedData.map(item => item.flowRate)).toLocaleString()} L
              </p>
            </div>
          </div>
        </div>
